...
class TextureAtlas:
    ...
    def add_texture(
        self, file_name: str, image: PIL.Image.Image
    ) -> typing.Tuple[int, int]:
        if file_name in self.file2position:
            return self.file2position[file_name]
    ...
    async def async_add_texture(self, file_name: str):
        ...
        image = await shared.async_side_instance.sided_task_manager.invokeOn(
            "data_processing", lookup_resource, file_name
        )
        return self.add_texture(file_name, image)

    def bake(self):
        self.foundation_image.save(shared.local + "/texture_atlas.png")
        self.loaded_pyglet_image = pyglet.image.load(
            shared.local + "/texture_atlas.png"
        ).get_texture()
