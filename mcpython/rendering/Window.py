...
class Window(pyglet.window.Window):
    ...
    def __init__(self, process_handler):
        ...
        self.version_info_label = pyglet.text.Label(
            text=f"MCPYTHON PRE-ALPHA " + shared.VERSION_NAME,
            color=(0, 0, 0, 255),
            batch=self.label_batch,
        )
        self.fps_label = pyglet.text.Label(
            text="fps: -", color=(0, 0, 0, 255), batch=self.label_batch
        )

    def on_draw(self):
        self.clear()
        pyglet.gl.glClearColor(1, 1, 1, 1)
        self.version_info_label.y = (
            self.get_size()[1] - self.version_info_label.content_height - 10
        )
        self.fps_label.text = "fps: " + str(pyglet.clock.get_fps())
        self.fps_label.y = (
            self.get_size()[1]
            - self.version_info_label.content_height
            - self.fps_label.content_height
            - 20
        )
