...
class RenderingTarget(enum.Enum):
    ...
    todo: extendable
    todo: allocation in ChunkSectionCache dynamic based on registered types, dynamically when needed for section rendering
    """

    # The default rendering, with depth test, and no alpha
    DEFAULT = 0

    # With alpha
    ALPHA = 1

    # todo: add overlay (render without depth test at end), background (overlay at start)
...
class ChunkSectionCache:
    ...
    todo: can we used index variant in some cases?
    """

    def __init__(
        self,
        position: typing.Tuple[int, int],
        redraw_callback: typing.Callable[["ChunkSectionCache"], None] = None,
    ):
    ...
    def allocate_new(
        self,
        vertices: typing.Iterable[float],
        texture_coordinates: typing.Iterator[float],
        target=RenderingTarget.DEFAULT,
    ) -> int:
        ...
        buffer = (
            self.normal_vertex_buffer if target.value == 0 else self.alpha_vertex_buffer
        )
    ...
    def redraw(self, warn_on_missing_callback=True):
        ...
        if callable(self.redraw_callback):
            ...
            print(
                f"[CHUNK SECTION CACHE RENDERER][WARN] section {self.position} has no set callback, but was scheduled for redraw"
            )
